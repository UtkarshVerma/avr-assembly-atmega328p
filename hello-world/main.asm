...
main:
  ...
  ; Shift address 1 bit to the left to tell `lpm` to fetch the lower byte
  ldi zl, low(message<<1)
  ldi zh, high(message<<1)
printString:
  lpm r16, z          ; Load character pointed to by `z` into `r16`
  cpi r16, 0          ; Check if character is non-zero
  breq loop
  rcall printChar     ; Print character if it's non-zero
...
lcdInstruction:
  cbi PORTD, log2(RS) ; Clear RS
  rcall writeByte
  ret

printChar:
  sbi PORTD, log2(RS) ; Set RS
  rcall writeByte
  ret
...
pulseEnable:
  cbi PORTD, log2(EN) ; Pull EN LOW
  delayUS 1

  sbi PORTD, log2(EN) ; Pull EN HIGH
  delayUS 1           ; Pulse must be wider than 450ns

  cbi PORTD, log2(EN) ; Pull EN LOW again
